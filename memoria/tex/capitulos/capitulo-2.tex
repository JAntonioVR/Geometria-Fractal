...
\begin{proof}
    ...
    $$
    de donde deducimos que $\{x_n\}$ es una sucesión de Cauchy, y como $X$ es completo, tenemos que existe $x\in X$ tal que $\{x_{n+1}\}=\{f(x_n)\}=\{f^n(x_0)\}\rightarrow x$. Pero $f$ es continua, por lo que si $\{x_n\}\rightarrow x$, necesariamente $\{f(x_n)\}\rightarrow f(x)$, por lo que $f(x)=x$.
