...
Lo primero es fijar una serie de conceptos. Recordamos que denominábamos \textit{lookfrom} al punto desde el cual se observa, es decir, la posición del espectador; y \textit{lookat} al punto sobre el cual se fija la mirada. Se denomina \textit{field of view (FOV)} al ángulo total observable, que corresponde al ángulo $\theta$ en la imagen \ref{fig:fov}. Como nuestro plano de proyección no es cuadrado, este ángulo es distinto en vertical y en horizontal, pero del ratio $16:9$ se deduce el uno del otro. En nuestro caso fijaremos $\theta=90^\circ=\pi/2 rad$, sea $h=\tan\left(\frac \theta 2\right)$ el ratio constante que van a mantener la semialtura del plano de proyección y la distancia focal, tal y como se puede observar en la imagen \ref{fig:fov}. Esto implica que la altura del plano de proyección (\verb|viewport_height|) es $PP_{height}=2h$, por lo que la anchura es 
$PP_width=AR\cdot PP_{height}$. 

\begin{figure} [ht]
    \centering
    \includegraphics[scale = 0.35]{img/C7/fov.png}
    \caption{Field Of View}
    \label{fig:fov}
\end{figure}

Pensemos por otro lado que aunque definamos un punto desde el que mirar y un punto al que mirar, no está todo dicho sobre la orientación de la cámara. Piense que mientras usted lee este documento puede girar la cabeza hacia un lado la cabeza y seguir mirando desde y hacia la misma posición. Por tanto necesitamos alguna de expresarle a la cámara que se mantenga vertical. Esto se hace mediante un vector que se denomina \textit{view up (vup)}. Normalmente, por convención se hace la elección $vup=(0,1,0)$. Mediante el vector $VD=lookat-lookfrom$ \textit{view direction} y el vector $vup$ podemos crear una base ortogonal del plano ortogonal a $VD$ que pasa por el punto $lookfrom$ y que define la orientación de la cámara
\begin{eqnarray*}
...
\begin{figure} [ht]
    \centering
    \includegraphics[scale = 0.35]{img/C7/plano.png}
